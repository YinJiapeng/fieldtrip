...
switch dataformat
    case {'matlab'} % jp modified
        [p, ~, ~] = fileparts(datafile);
        pn_fp_data = fullfile(p, 'fp_data');
        f_list = dirr(pn_fp_data, '.mat');
        
        n_samples = hdr.nSamples;
        n_ch = length(chanindx);
        dat = [];
        for i = 1:n_ch
            idx_ch = chanindx(i);
            pne = fullfile(pn_fp_data, f_list(idx_ch).name);
            tmp_data = load(pne);
            dat(i,:) = tmp_data.data(begsample:endsample);
        end
